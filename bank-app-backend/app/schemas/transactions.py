...
class TransactionCreateRequest(BaseModel):
    recipient_account_number: str = Field(..., description="The account number of the recipient.")
    recipient_name: Optional[str] = Field(None, description="Name of the recipient for transaction notifications and SMS.")
    amount: float = Field(..., gt=0, description="The amount to transfer.")
    terminal_id: str = Field(..., description="A unique identifier for the client device/terminal.")
...
class TransactionResponse(BaseModel):
    status: str
    new_balance: Optional[float] = None
    transaction_id: Optional[str] = None  # Added for SMS tracking
    fraud_prediction: bool = False
    fraud_probability: Optional[float] = None
    fraud_details: Optional[dict] = None
    blocked: bool = False
    ...
    auth_method: Optional[str] = None
    fraud_detection_bypassed: bool = False
    sms_sent: Optional[bool] = None  # Added to track SMS delivery status
    restoration_info: Optional[dict] = None  # Added for restoration limit info
