...
const SignatureVerification = ({ onProceed, phoneNumber, customerId }: SignatureVerificationProps) => {
  ...
  const submitSignature = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/v1/restore/signature', {
        method: 'POST',
      ...
      setError(`Failed to verify signature: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
    ...
    <div className="min-h-screen bg-gradient-surface flex items-center justify-center p-4">
      <Card className="w-full max-w-md p-8 shadow-card animate-slide-up">
        <div className="flex items-center mb-6">
          <h1 className="text-2xl font-bold text-foreground">Verify Your Signature</h1>
        </div>
